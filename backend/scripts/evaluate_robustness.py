...
def load_data_and_model(data_path=None, model_path=None):
    ...
    logger.info(f"Loading data from: {data_path}")  # OK: has curly braces
    logger.info(f"Loading model from: {model_path}")  # OK: has curly braces
...
def prepare_features(df, feature_names):
    ...
    if missing_features:
        logger.warning(f"Missing features (will be filled with 0): {missing_features}")  # OK: has curly braces
...
def temporal_validation(data, model, feature_names):
    ...
    logger.info(f"Training samples (2022-2024): {len(train_data)}")  # OK: has curly braces
    logger.info(f"Test samples (2025): {len(test_data)}")  # OK: has curly braces
    ...
    logger.info(f"  Accuracy:  {metrics['accuracy']:.4f}")  # OK: has curly braces
    logger.info(f"  Precision: {metrics['precision']:.4f}")  # OK: has curly braces
    logger.info(f"  Recall:    {metrics['recall']:.4f}")  # OK: has curly braces
    logger.info(f"  F1 Score:  {metrics['f1_score']:.4f}")  # OK: has curly braces
    if "roc_auc" in metrics:
        logger.info(f"  ROC-AUC:   {metrics['roc_auc']:.4f}")  # OK: has curly braces
        logger.info(f"  Brier:     {metrics['brier_score']:.4f}")  # OK: has curly braces
...
def robustness_testing(data, model, feature_names, noise_levels=[0.05, 0.10, 0.15, 0.20]):
    ...
    logger.info(f"\nBaseline (no noise):")
    logger.info(f"  Accuracy: {baseline_acc:.4f}")  # OK: has curly braces
    logger.info(f"  F1 Score: {baseline_f1:.4f}")  # OK: has curly braces

    results.append(
        {"noise_level": 0.0, "accuracy": baseline_acc, "f1_score": baseline_f1, "accuracy_drop": 0.0, "f1_drop": 0.0}
    )
...
def probability_calibration_analysis(data, model, feature_names):
    ...
    logger.info(f"  Min:    {y_pred_proba.min():.4f}")  # OK: has curly braces
    logger.info(f"  Max:    {y_pred_proba.max():.4f}")  # OK: has curly braces
    logger.info(f"  Mean:   {y_pred_proba.mean():.4f}")  # OK: has curly braces
    logger.info(f"  Median: {np.median(y_pred_proba):.4f}")  # OK: has curly braces
    logger.info(f"  Std:    {y_pred_proba.std():.4f}")  # OK: has curly braces
    ...
    logger.info(
        f"  High confidence flood (>90%):     {high_conf_flood} ({high_conf_flood/len(y)*100:.1f}%)"
    )  # OK: has curly braces
    logger.info(
        f"  High confidence no-flood (<10%):  {high_conf_no_flood} ({high_conf_no_flood/len(y)*100:.1f}%)"
    )  # OK: has curly braces
    logger.info(
        f"  Uncertain (30-70%):               {uncertain} ({uncertain/len(y)*100:.1f}%)"
    )  # OK: has curly braces

    # Brier score (lower is better, 0 is perfect)
    brier = brier_score_loss(y, y_pred_proba)
    logger.info(f"\nBrier Score: {brier:.6f} (0=perfect, 0.25=random)")  # OK: has curly braces
    ...
    try:
        ...
        for pt, pp in zip(prob_true, prob_pred):
            logger.info(f"  Predicted: {pp:.2f} -> Actual: {pt:.2f}")  # OK: has curly braces
    except Exception as e:
        logger.warning(f"Could not compute calibration curve: {e}")  # OK: has curly braces
...
def feature_threshold_analysis(data):
    ...
    logger.info(f"    Count: {len(no_flood_data)}")  # OK: has curly braces
    logger.info(f"    Mean:  {no_flood_data.mean():.2f} mm")  # OK: has curly braces
    logger.info(f"    Std:   {no_flood_data.std():.2f} mm")  # OK: has curly braces
    logger.info(f"    Min:   {no_flood_data.min():.2f} mm")  # OK: has curly braces
    logger.info(f"    Max:   {no_flood_data.max():.2f} mm")  # OK: has curly braces

    logger.info(f"\n  Flood (class 1):")
    logger.info(f"    Count: {len(flood_data)}")  # OK: has curly braces
    logger.info(f"    Mean:  {flood_data.mean():.2f} mm")  # OK: has curly braces
    logger.info(f"    Std:   {flood_data.std():.2f} mm")  # OK: has curly braces
    logger.info(f"    Min:   {flood_data.min():.2f} mm")  # OK: has curly braces
    logger.info(f"    Max:   {flood_data.max():.2f} mm")  # OK: has curly braces
    ...
    if overlap_max > overlap_min:
        overlap_records = data[(data["precipitation"] >= overlap_min) & (data["precipitation"] <= overlap_max)]
        logger.info(
        ...
        logger.info(f"  Threshold: ~{no_flood_data.max():.1f} - {flood_data.min():.1f} mm")  # OK: has curly braces

    # This explains the 100% accuracy
    gap = flood_data.min() - no_flood_data.max()
    if gap > 0:
        logger.info(f"\n  GAP between classes: {gap:.2f} mm")  # OK: has curly braces
...
def cross_validation_analysis(data, model, feature_names, n_folds=5):
    ...
    logger.info(f"  F1 Score:  {f1_scores.mean():.4f} (+/- {f1_scores.std()*2:.4f})")  # OK: has curly braces
    logger.info(
        f"  Precision: {precision_scores.mean():.4f} (+/- {precision_scores.std()*2:.4f})"
    )  # OK: has curly braces
    logger.info(f"  Recall:    {recall_scores.mean():.4f} (+/- {recall_scores.std()*2:.4f})")  # OK: has curly braces
    ...
    for i in range(n_folds):
        logger.info(
            f"  {i+1:4d} | {accuracy_scores[i]:.4f}   | {f1_scores[i]:.4f}   | {precision_scores[i]:.4f}    | {recall_scores[i]:.4f}"
        )  # OK: has curly braces
...
def main():
    ...
    parser.add_argument("--output", type=str, default="evaluation_report.json", help="Output file for results")
    args = parser.parse_args()

    # Load data and model
    data, model, metadata = load_data_and_model(data_path=args.data_path, model_path=args.model_path)

    # Get feature names from model (preferred) or metadata
    if hasattr(model, "feature_names_in_"):
        feature_names = list(model.feature_names_in_)
        logger.info(f"Using features from model: {feature_names}")
    else:
