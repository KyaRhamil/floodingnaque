...
PAGASA_FEATURE_GROUPS = {
    ...
    'derived': [
        ...
        'temp_precip_interaction', 'monsoon_precip_interaction',
        'wind_rain_interaction', 'saturation_risk'
    ],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train flood prediction model using PAGASA weather data'
    )
    parser.add_argument(
        '--data-path',
        type=str,
        help='Path to training CSV (default: auto-generated PAGASA dataset)'
    )
    parser.add_argument(
        '--all-stations',
        action='store_true',
        help='Use data from all 3 PAGASA stations (default: NAIA only)'
    )
    parser.add_argument(
        '--grid-search',
        action='store_true',
    ...
    parser.add_argument(
        '--production',
        action='store_true',
        help='Full production pipeline (grid search + SHAP + report)'
    )
    parser.add_argument(
        '--reprocess',
        action='store_true',
        help='Force reprocessing of PAGASA data'
    )
    parser.add_argument(
        '--version',
        type=int,
