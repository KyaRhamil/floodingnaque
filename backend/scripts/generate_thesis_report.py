...
def plot_feature_importance(model, output_dir, top_n=20):
    ...
    plt.title("Random Forest Feature Importance\nFlood Prediction Model", fontsize=14, fontweight="bold", pad=20)
    plt.gca().invert_yaxis()

    # Add value labels
    for i, (idx, row) in enumerate(importance_df.iterrows()):
        plt.text(row["importance"], i, f' {row["importance"]:.3f}', va="center", fontsize=9)
...
def plot_confusion_matrix(y_true, y_pred, output_dir):
    ...
    sns.heatmap(
        cm, annot=True, fmt="d", cmap="Blues", cbar_kws={"label": "Count"}, annot_kws={"size": 14, "weight": "bold"}
    )
...
def plot_learning_curves(model, X, y, output_dir):
    ...
    plt.title("Learning Curves\nRandom Forest Flood Prediction Model", fontsize=14, fontweight="bold", pad=20)
    plt.legend(loc="lower right", fontsize=11)
    plt.grid(True, alpha=0.3)
...
def plot_model_comparison(metadata, output_dir):
    ...
    ax.set_title("Model Performance Metrics\nRandom Forest Flood Prediction", fontsize=14, fontweight="bold", pad=20)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate comprehensive thesis report for Random Forest model")
