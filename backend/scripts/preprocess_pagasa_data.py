...
def add_interaction_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "wind_direction" in df.columns:
        df["is_sw_monsoon_wind"] = ((df["wind_direction"] >= 180) & (df["wind_direction"] <= 270)).astype(int)
...
def merge_with_flood_records(weather_df: pd.DataFrame, flood_records_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    """
    ...
    if all(c in all_floods.columns for c in ["year", "month"]):
        ...
        weather_df["confirmed_flood_month"] = weather_df.apply(
            lambda x: 1 if (x["year"], x["month"]) in flood_months else 0, axis=1
        )
        ...
        weather_df.loc[(weather_df["confirmed_flood_month"] == 1) & (weather_df["precipitation"] > 10), "flood"] = 1
...
def create_training_dataset(use_naia_only: bool = True, include_flood_records: bool = True) -> pd.DataFrame:
    """
    ...
    all_training_features = (
        core_features + target_features + temporal_features + rolling_features + derived_features + spatial_features
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Preprocess PAGASA weather station data for flood prediction")
    parser.add_argument(
        "--station",
    ...
    parser.add_argument(
        "--merge-flood-records", action="store_true", help="Merge with official Parañaque flood records"
    )
    parser.add_argument("--create-training", action="store_true", help="Create final training dataset")
    parser.add_argument(
        "--naia-only", action="store_true", help="Use only NAIA station for training (closest to Parañaque)"
    )
    parser.add_argument("--summary", action="store_true", help="Print detailed data summary")
    ...
    if args.create_training:
        df = create_training_dataset(use_naia_only=args.naia_only, include_flood_records=True)
        if args.summary:
        ...
        df = process_single_station(args.station, merge_flood_records=args.merge_flood_records)
        if args.summary:
