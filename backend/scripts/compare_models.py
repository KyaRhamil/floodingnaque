...
def get_all_model_versions(models_dir="models"):
    ...
    for model_file in sorted(models_path.glob("flood_rf_model_v*.joblib")):
        try:
            ...
            versions.append({"version": version, "path": str(model_file), "metadata": metadata})
...
def plot_metrics_evolution(df, output_dir):
    ...
    for metric, color in zip(metrics, colors):
        plt.plot(
            df["version"], df[metric], marker="o", linewidth=2, label=metric.replace("_", " ").title(), color=color
        )

    plt.xlabel("Model Version", fontsize=12, fontweight="bold")
    plt.ylabel("Score", fontsize=12, fontweight="bold")
    plt.title(
        "Model Performance Evolution Across Versions\nFlood Prediction System", fontsize=14, fontweight="bold", pad=20
    )
    ...
    for metric in metrics:
        plt.annotate(
            f"{df[metric].iloc[latest_idx]:.3f}",
...
def plot_metrics_bars(df, output_dir):
    ...
    for i, (metric, color) in enumerate(zip(metrics, colors)):
        offset = width * (i - 1.5)
        ax.bar(
            [p + offset for p in x], df[metric], width, label=metric.replace("_", " ").title(), color=color, alpha=0.8
        )

    ax.set_xlabel("Model Version", fontsize=12, fontweight="bold")
    ax.set_ylabel("Score", fontsize=12, fontweight="bold")
    ax.set_title(
        "Model Metrics Comparison Across Versions\nFlood Prediction System", fontsize=14, fontweight="bold", pad=20
    )
    ax.set_xticks(x)
...
def plot_parameters_evolution(df, output_dir):
    ...
    # Plot n_estimators
    ax1.plot(df["version"], df["n_estimators"], marker="s", linewidth=2, color="#9b59b6", markersize=8)
    ...
    for idx, row in df.iterrows():
        ax2.text(
            row["version"], row["dataset_size"], f'{int(row["dataset_size"])}', ha="center", va="bottom", fontsize=9
        )

    plt.suptitle("Model Configuration Evolution\nFlood Prediction System", fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout()
...
Example:
  ...
  python compare_models.py --models-dir models --output comparison_results
    ...
    parser.add_argument("--models-dir", type=str, default="models", help="Directory containing model files")
    parser.add_argument("--output", type=str, default="reports", help="Output directory for comparison reports")
