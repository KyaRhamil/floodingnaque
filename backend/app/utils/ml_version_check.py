...
def validate_ml_versions(
    ...
    for package, expected_version in PINNED_VERSIONS.items():
        ...
        if actual_version is None:
            ...
            mode = VERSION_CHECK_STRICT if strict else PACKAGE_COMPATIBILITY_MODE.get(
                package, VERSION_CHECK_STRICT
            )
            is_compatible, message = check_version_compatibility(
                expected_version, actual_version, mode
            )
            ...
            if is_compatible:
                ...
                if package in ("scikit-learn", "joblib"):
                    ...
                    critical_errors.append(
                        f"CRITICAL: {package} version mismatch - {message}. "
    ...
    if raise_on_error and critical_errors:
        raise RuntimeError(
            "ML stack version validation failed:\n" + "\n".join(critical_errors)
        )
...
def get_ml_version_report() -> Dict:
    ...
    return {
        ...
        "installed_versions": {
            r.package: r.actual for r in results
        },
        ...
        "critical_issues": [
            r.message for r in results if r.severity == "error"
        ],
        "warnings": [
            r.message for r in results if r.severity == "warning"
        ]
    }
...
def log_ml_version_status(log_level: int = logging.INFO) -> None:
    ...
    if all_ok:
        ...
        if errors:
            logger.error(
                f"ML validation: {len(errors)} critical issues, {len(warnings)} warnings"
            )
        elif warnings:
...
def check_model_training_versions(model_metadata: Dict) -> List[str]:
    ...
    if not training_versions:
        warnings.append(
            "Model metadata does not contain training versions. "
            "Cannot verify ML stack compatibility."
        )
        return warnings
    ...
    for package, trained_version in training_versions.items():
        ...
        is_compatible, message = check_version_compatibility(
            trained_version, current_version, mode
        )
