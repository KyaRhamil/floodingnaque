...
class RateLimitTier:
    ...
    def __init__(self, name: str, requests_per_minute: int, requests_per_hour: int, 
                 ...
                 priority: int = 0, degradation_factor: float = 0.5):
        self.name = name
...
class IPReputationManager:
    ...
    def record_malicious_activity(self, ip_address: str, reason: str = '') -> None:
        ...
        logger.error(
            f"Malicious activity from {ip_address}: {reason} "
    ...
    def cleanup_old_entries(self, max_age_hours: int = 24) -> int:
        ...
        to_remove = [
            ip for ip, entry in self._ip_data.items()
            if entry.last_seen < cutoff and entry.score >= self.GOOD_THRESHOLD
        ]
...
def get_rate_limit_for_key(api_key_hash: str, limit_type: str = 'per_minute',
