...
ENV_VAR_SPECS: List[EnvVarSpec] = [
    ...
    EnvVarSpec(
        name="JWT_SECRET_KEY", required_in_prod=True, sensitive=True, min_length=32, description="JWT signing key"
    ),
    # Database
    EnvVarSpec(
        name="DATABASE_URL",
    ...
    # API Keys
    EnvVarSpec(
        name="OWM_API_KEY", required_in_prod=True, sensitive=True, min_length=16, description="OpenWeatherMap API key"
    ),
    EnvVarSpec(
    ...
    EnvVarSpec(name="WORLDTIDES_API_KEY", sensitive=True, description="WorldTides API key for tidal data"),
    EnvVarSpec(
        name="INTERNAL_API_TOKEN", sensitive=True, min_length=32, description="Internal service authentication token"
    ),
    ...
    EnvVarSpec(name="HOST", default="0.0.0.0", description="Server host"),  # nosec B104
    # CORS
    EnvVarSpec(name="CORS_ORIGINS", required_in_prod=True, description="Allowed CORS origins (comma-separated)"),
    # Rate Limiting
    EnvVarSpec(
    ...
    EnvVarSpec(name="RATE_LIMIT_STORAGE_URL", default="memory://", description="Rate limit storage backend URL"),
    ...
    EnvVarSpec(
        name="LOG_FORMAT", default="json", allowed_values=["json", "ecs", "text"], description="Log output format"
    ),
    # Sentry
    EnvVarSpec(
        name="SENTRY_DSN",
    ...
    # Model Configuration
    EnvVarSpec(name="MODEL_DIR", default="models", description="Directory containing ML models"),
    EnvVarSpec(name="MODEL_NAME", default="flood_rf_model", description="Name of the prediction model"),
    # Health Check SLA
    EnvVarSpec(
...
def validate_all_env_vars(
    additional_specs: Optional[List[EnvVarSpec]] = None, raise_on_critical: bool = True, log_results: bool = True
) -> ValidationReport:
    """
