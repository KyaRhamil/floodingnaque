...
correlation_context: ContextVar[Optional['CorrelationContext']] = ContextVar(
    'correlation_context', 
    default=None
)
...
def generate_correlation_id() -> str:
    ...
    """
    timestamp_hex = hex(int(datetime.utcnow().timestamp() * 1000))[2:]
...
class CorrelationContext:
    ...
    def from_headers(cls, headers: Dict[str, str], **kwargs) -> 'CorrelationContext':
        ...
        correlation_id = (
            normalized.get('x-correlation-id') or
            normalized.get('x-request-id') or
