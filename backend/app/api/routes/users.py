    ...
    try:
        ...
        with get_db_session() as session:
            ...
            if not verify_password(password, user.password_hash):
                ...
                logger.warning(f"Failed login attempt for {email} (attempt {user.failed_login_attempts}) [{request_id}]")
                return api_error('InvalidCredentials', 'Invalid email or password', HTTP_UNAUTHORIZED, request_id)
...
def confirm_password_reset():
    ...
    try:
        ...
        if not email or not token or not new_password:
            return api_error('ValidationError', 'Email, token, and new password are required', HTTP_BAD_REQUEST, request_id)
