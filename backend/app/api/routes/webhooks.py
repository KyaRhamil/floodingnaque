...
from app.utils.api_constants import HTTP_OK, HTTP_CREATED, HTTP_BAD_REQUEST, HTTP_NOT_FOUND, HTTP_INTERNAL_ERROR
import logging
import secrets
import json
import hashlib
...
def calculate_backoff_delay(attempt: int, initial_delay: float = WEBHOOK_INITIAL_DELAY_SEC, 
                            multiplier: float = WEBHOOK_BACKOFF_MULTIPLIER,
                            max_delay: float = WEBHOOK_MAX_DELAY_SEC,
                            jitter_factor: float = WEBHOOK_JITTER_FACTOR) -> float:
    """
...
def deliver_webhook(webhook_url: str, webhook_secret: str, payload: WebhookPayload,
                    timeout: int = WEBHOOK_TIMEOUT_SEC) -> WebhookDeliveryResult:
    """
    ...
    try:
        ...
        return WebhookDeliveryResult(
            success=False,
            error_message=f"Delivery failed: {str(e)[:100]}",
            attempt=payload.attempt
        )


def deliver_webhook_with_retry(webhook_id: int, event_type: str, data: Dict[str, Any],
                               max_retries: int = WEBHOOK_MAX_RETRIES,
                               async_delivery: bool = True) -> Optional[WebhookDeliveryResult]:
    """
    ...
    def _deliver():
        ...
        for attempt in range(max_retries + 1):
            ...
            if attempt < max_retries:
                ...
                logger.error(
                    f"Webhook {webhook_id} delivery failed after {max_retries + 1} attempts: "
        ...
        with get_db_session() as session:
            ...
            if wh:
                ...
                if wh.failure_count >= WEBHOOK_MAX_FAILURE_COUNT:
                    wh.is_active = False
                    logger.warning(
                        f"Webhook {webhook_id} auto-disabled after {wh.failure_count} consecutive failures"
                    )
...
def trigger_webhook_event():
    ...
    try:
        ...
        if event_type not in valid_events:
            return api_error('ValidationError', f'Invalid event_type. Valid: {valid_events}', HTTP_BAD_REQUEST, request_id)
...
def get_webhook_config():
    ...
    return jsonify({
        ...
        'config': {
            ...
            'retry_delays_preview': [
                calculate_backoff_delay(i) for i in range(WEBHOOK_MAX_RETRIES)
            ]
        },
        'valid_events': ['flood_detected', 'critical_risk', 'high_risk', 'medium_risk', 'low_risk'],
        'request_id': request_id
