...
OPTIONAL_COLUMNS = {
    'wind_speed', 'pressure', 'source', 'location_lat', 'location_lon',
...
def _parse_row(row: Dict[str, str], header_map: Dict[str, str], row_num: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if 'could not convert' in str(e).lower():
            raise ValueError(f'Invalid humidity value')
        raise
    ...
    try:
        ...
        if parsed['precipitation'] < 0:
            raise ValueError(f'Precipitation cannot be negative')
    except ValueError as e:
        if 'could not convert' in str(e).lower():
            raise ValueError(f'Invalid precipitation value')
        raise
    ...
    for field in OPTIONAL_COLUMNS:
        ...
        if value:
            try:
                if field in ('wind_speed', 'pressure', 'location_lat', 'location_lon',
                            'tide_height', 'tide_risk_factor', 'hours_until_high_tide',
                            'satellite_precipitation_rate', 'precipitation_1h',
                            'precipitation_3h', 'precipitation_24h', 'data_quality'):
...
def parse_excel_content(file_content: bytes) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    try:
        ...
        if len(df) > MAX_ROWS_PER_UPLOAD:
            errors.append(f'Maximum row limit ({MAX_ROWS_PER_UPLOAD}) exceeded, processing first {MAX_ROWS_PER_UPLOAD} rows')
...
def _parse_excel_row(row, row_num: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if parsed['temperature'] < 173.15 or parsed['temperature'] > 333.15:
            raise ValueError(f'Temperature out of valid range (173.15K - 333.15K)')
    ...
    try:
        ...
        if parsed['humidity'] < 0 or parsed['humidity'] > 100:
            raise ValueError(f'Humidity out of valid range (0-100)')
    except (ValueError, TypeError) as e:
...
def upload_csv():
    ...
    try:
        ...
        if file_size > MAX_FILE_SIZE_MB * 1024 * 1024:
            return api_error('ValidationError', f'File exceeds maximum size of {MAX_FILE_SIZE_MB}MB', HTTP_BAD_REQUEST, request_id)
        ...
        if not valid_rows and errors:
            return api_error('ValidationError', 'No valid rows found', HTTP_BAD_REQUEST, request_id, details={'errors': errors})
        
        if errors and not skip_errors:
            return api_error('ValidationError', 'Validation errors found', HTTP_BAD_REQUEST, request_id, details={'errors': errors, 'valid_rows': len(valid_rows)})
...
def upload_excel():
    ...
    try:
        ...
        if not (file.filename.lower().endswith('.xlsx') or file.filename.lower().endswith('.xls')):
            return api_error('ValidationError', 'File must be an Excel file (.xlsx or .xls)', HTTP_BAD_REQUEST, request_id)
...
def validate_upload():
    ...
    try:
        ...
        if filename.endswith('.csv'):
            ...
            return api_error('ValidationError', 'Unsupported file format. Use CSV or Excel files.', HTTP_BAD_REQUEST, request_id)
