...
from app.utils.api_constants import HTTP_OK, HTTP_CREATED, HTTP_BAD_REQUEST, HTTP_NOT_FOUND, HTTP_INTERNAL_ERROR, HTTP_SERVICE_UNAVAILABLE
import logging
import os
...
def create_weather_data():
    ...
    try:
        ...
        if temperature < 173.15 or temperature > 333.15:
            return api_error('ValidationError', 'Temperature must be between 173.15K and 333.15K', HTTP_BAD_REQUEST, request_id)
        ...
        if timestamp_str:
            try:
                ...
                return api_error('ValidationError', 'Invalid timestamp format. Use ISO format.', HTTP_BAD_REQUEST, request_id)
        else:
...
def bulk_delete_weather_data():
    ...
    try:
        ...
        if not ids and not start_date and not end_date and not source_filter:
            return api_error('ValidationError', 'At least one filter (ids, date range, or source) must be provided', HTTP_BAD_REQUEST, request_id)
