...
from app.models.db import AlertHistory, Prediction, get_db_session
        ...
        with get_db_session() as session:
            ...
            if status:
                ...
                if status not in valid_statuses:
                    return api_error('ValidationError', f'Status must be one of: {valid_statuses}', HTTP_BAD_REQUEST, request_id)
...
def get_alert_stats():
    ...
    try:
        ...
        with get_db_session() as session:
            ...
            total_all_time = session.query(AlertHistory).filter(
                AlertHistory.is_deleted == False
            ).count()
            ...
            # Alerts this week
            alerts_week = session.query(AlertHistory).filter(
                AlertHistory.is_deleted == False,
                AlertHistory.created_at >= week_ago
            ).count()
            
            # Alerts this month
            alerts_month = session.query(AlertHistory).filter(
                AlertHistory.is_deleted == False,
                AlertHistory.created_at >= month_ago
            ).count()
            
            # Critical alerts (risk level 2) in last 24 hours
            critical_24h = session.query(AlertHistory).filter(
                AlertHistory.is_deleted == False,
            ...
            # Most recent alert
            latest_alert = session.query(AlertHistory).filter(
                AlertHistory.is_deleted == False
            ).order_by(desc(AlertHistory.created_at)).first()
