...
class SatelliteWeatherCache(Base):
    ...
    # Cache metadata
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    expires_at = Column(DateTime(timezone=True), nullable=False, index=True, info={'description': 'Cache expiration time'})
    is_valid = Column(Boolean, default=True, nullable=False, index=True, info={'description': 'Whether cache entry is still valid'})
    ...
    __table_args__ = (
        ...
        CheckConstraint('era5_humidity IS NULL OR (era5_humidity >= 0 AND era5_humidity <= 100)', name='swc_valid_humidity'),
    ...
    def __repr__(self):
        return f"<SatelliteWeatherCache(id={self.id}, lat={self.latitude}, lon={self.longitude}, dataset={self.dataset})>"
...
class TideDataCache(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint('tide_risk_factor IS NULL OR (tide_risk_factor >= 0 AND tide_risk_factor <= 1)', name='tdc_valid_risk_factor'),
