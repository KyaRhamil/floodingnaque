...
class AsyncWorldTidesService:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            self._session = aiohttp.ClientSession(
                ...
                headers={
                    'User-Agent': 'FloodingNaque/1.0 (Flood Prediction System)'
                }
            )
    ...
    async def _make_request(self, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        headers = inject_correlation_headers({
            'User-Agent': 'FloodingNaque/2.0 (Flood Prediction System)'
        })
    ...
    async def get_tide_data_for_prediction(
        ...
        result['tide_risk_factor'] = self._calculate_tide_risk_factor(
            current.height, 
            extremes
        )
    ...
    def _calculate_tide_risk_factor(
        self,
...
async def get_current_tide_async(
    lat: Optional[float] = None,
    lon: Optional[float] = None
) -> Optional[TideData]:
    """
