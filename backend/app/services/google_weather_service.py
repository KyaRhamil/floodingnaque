...
class GoogleWeatherService:
    ...
    def get_gpm_precipitation(
        ...
        try:
            ...
            # Load GPM collection
            collection = ee.ImageCollection(self.DATASETS['GPM']['collection']) \
    ...
    def get_chirps_precipitation(
        ...
        try:
            ...
            collection = ee.ImageCollection(self.DATASETS['CHIRPS']['collection']) \
                .filterDate(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')) \
                .filterBounds(point)
    ...
    def get_era5_weather(
        ...
        try:
            ...
            collection = ee.ImageCollection(self.DATASETS['ERA5']['collection']) \
                .filterDate(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d')) \
                .filterBounds(point)
            ...
            for feature in info.get('features', []):
                ...
                results.append(WeatherReanalysis(
...
def get_google_weather_for_ingest(
    lat: Optional[float] = None,
    lon: Optional[float] = None
) -> Optional[Dict[str, Any]]:
    """
