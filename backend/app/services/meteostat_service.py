...
from app.utils.circuit_breaker import meteostat_breaker, CircuitOpenError
...
class MeteostatService:
    ...
    def get_historical_for_training(
        ...
        try:
            ...
            result = pd.DataFrame({
                'temperature': df['tavg'].apply(lambda x: x + 273.15 if pd.notna(x) else None),  # Convert to Kelvin
