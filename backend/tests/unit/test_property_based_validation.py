...
from tests.strategies import (
    valid_temperature, valid_humidity, valid_precipitation,
    valid_latitude, valid_longitude, weather_data,
    invalid_weather_data, extreme_weather_data,
    sql_injection_string, xss_string, path_traversal_string,
    pagination_params, invalid_pagination_params
)
from app.utils.validation import (
    InputValidator, sanitize_input, check_path_traversal, ValidationError
)
...
class TestPropertyBasedWeatherValidation:
    ...
    def test_valid_humidity_always_passes(self, humidity):
        ...
        result = InputValidator.validate_float(
            humidity,
            'humidity',
            min_val=InputValidator.HUMIDITY_MIN,
            max_val=InputValidator.HUMIDITY_MAX
        )
    ...
    def test_valid_precipitation_always_passes(self, precip):
        ...
        result = InputValidator.validate_float(
            precip,
            'precipitation',
            min_val=InputValidator.PRECIPITATION_MIN,
            max_val=InputValidator.PRECIPITATION_MAX
        )
    ...
    def test_weather_data_dict_validation(self, data):
        ...
        temp = InputValidator.validate_float(
            data['temperature'], 'temperature',
            min_val=-50, max_val=50
        )
        humidity = InputValidator.validate_float(
            data['humidity'], 'humidity',
            min_val=0, max_val=100
        )
        precip = InputValidator.validate_float(
...
class TestPropertyBasedLocationValidation:
    ...
    def test_valid_latitude_always_passes(self, lat):
        ...
        result = InputValidator.validate_float(
            lat, 'latitude',
            min_val=InputValidator.LATITUDE_MIN,
            max_val=InputValidator.LATITUDE_MAX
        )
        assert result == lat
    ...
    def test_valid_longitude_always_passes(self, lon):
        ...
        result = InputValidator.validate_float(
            lon, 'longitude',
            min_val=InputValidator.LONGITUDE_MIN,
            max_val=InputValidator.LONGITUDE_MAX
        )
        assert result == lon
...
class TestPropertyBasedPaginationValidation:
    ...
    def test_valid_pagination_params(self, params):
        ...
        limit = InputValidator.validate_int(
            params['limit'], 'limit',
            min_val=1, max_val=1000
        )
        offset = InputValidator.validate_int(
