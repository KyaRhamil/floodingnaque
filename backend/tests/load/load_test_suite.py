...
def check_thresholds(stats: Dict, thresholds: Dict[str, float]) -> Tuple[bool, List[str]]:
    """
    ...
    if "avg_response_time_ms" in thresholds:
        ...
        if avg_rt > thresholds["avg_response_time_ms"]:
            violations.append(
                f"Average response time {avg_rt:.0f}ms > " f"threshold {thresholds['avg_response_time_ms']}ms"
            )
    ...
    if "p99_response_time_ms" in thresholds:
        ...
        if p99 > thresholds["p99_response_time_ms"]:
            violations.append(f"P99 response time {p99:.0f}ms > " f"threshold {thresholds['p99_response_time_ms']}ms")
    ...
    if "failure_rate_percent" in thresholds:
        ...
        if failure_rate > thresholds["failure_rate_percent"]:
            violations.append(f"Failure rate {failure_rate:.2f}% > " f"threshold {thresholds['failure_rate_percent']}%")

    # Check minimum RPS
    if "requests_per_second_min" in thresholds:
        rps = total_stats.get("current_rps", 0)
        if rps < thresholds["requests_per_second_min"]:
            violations.append(f"RPS {rps:.1f} < minimum threshold {thresholds['requests_per_second_min']}")
...
def run_load_test(config: TestConfig, host: str, output_dir: Path, verbose: bool = False) -> TestResult:
    """
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=not verbose, text=True, cwd=str(Path(__file__).parent.parent.parent)
        )
    ...
    if stats:
        passed, violations = check_thresholds(stats, config.performance_thresholds or {})
...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive Load Testing Suite for Floodingnaque API")
    parser.add_argument(
        "--test-type",
    ...
    parser.add_argument("--output-dir", default="reports/load_tests", help="Directory for output files")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument("--ci", action="store_true", help="CI mode: exit with non-zero code if tests fail")
