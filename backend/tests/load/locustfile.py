...
class FloodPredictionAPIUser(HttpUser):
    ...
    def predict_normal_conditions(self):
        ...
        with self.client.post(
            "/predict?risk_level=true", json=payload, headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                data = response.json()
    ...
    def predict_rainy_conditions(self):
        ...
        with self.client.post(
            "/predict?risk_level=true&return_proba=true", json=payload, headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                data = response.json()
    ...
    def get_data_paginated(self):
        ...
        with self.client.get(f"/data?limit={limit}&offset={offset}", catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
...
class AuthenticatedAPIUser(HttpUser):
    ...
    def authenticated_data_access(self):
        ...
        with self.client.get("/data?limit=50", headers=self.headers, catch_response=True) as response:
    ...
    def batch_prediction(self):
        ...
        with self.client.post(
            "/batch/predict", json={"items": batch_data}, headers=self.headers, catch_response=True
        ) as response:
