...
def coordinates(draw) -> Dict[str, float]:
    ...
    """
    return {"lat": draw(valid_latitude()), "lon": draw(valid_longitude())}
...
def extreme_weather_data(draw) -> Dict[str, Any]:
    ...
    scenarios = draw(
        st.sampled_from(
            ["extreme_cold", "extreme_hot", "extreme_humid", "extreme_dry", "extreme_precipitation", "no_precipitation"]
        )
    )
    ...
    if scenarios == "extreme_cold":
        ...
        return {"temperature": draw(valid_temperature()), "humidity": draw(valid_humidity()), "precipitation": 0.0}
...
def probability_dict(draw) -> Dict[str, float]:
    ...
    return {"no_flood": 1.0 - flood_prob, "flood": flood_prob}
...
def model_prediction_output(draw) -> Dict[str, Any]:
    ...
    return {
        ...
        "probability": {"no_flood": 1.0 - flood_prob, "flood": flood_prob},
